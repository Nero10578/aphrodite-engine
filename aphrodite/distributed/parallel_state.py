...
class GroupCoordinator:
    ...
    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        ...
        if self.use_custom_op_call:
            return torch.ops.aphrodite.all_gather(input_,
                                             dim,
                                             world_size,
                                             group_name=self.unique_name)
        else:
    ...
    def reduce_scatter(self,
        ...
        if self.use_custom_op_call:
            return torch.ops.aphrodite.reduce_scatter(input_,
                                                 dim,
                                                 world_size,
                                                 group_name=self.unique_name)
        else:
...
@deprecated("`get_pipeline_model_parallel_group` has been replaced with "
            "`get_pp_group` and may be removed in a later version. Please use "
...
def init_distributed_environment(
    ...
    if _WORLD is None:
        ...
        logger.debug("Detected {} nodes in the distributed environment",
                     _NODE_COUNT)
    else:
