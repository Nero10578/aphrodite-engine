...
from aphrodite.lora.layers import BaseLayerWithLoRA, FusedMoEWithLoRA, LoRAMapping
...
from aphrodite.modeling.models.interfaces import is_pooling_model
from aphrodite.modeling.models.module_mapping import MultiModelKeys
from aphrodite.modeling.models.utils import PPMissingLayer, WeightsMapper
from aphrodite.utils.cache import LRUCache
        ...
        for tensor_name, tensor in tensors.items():
            ...
            if module_name not in loras:
                ...
                if embeddings:
                    ...
                    if embeddings_module:
                        lora_embeddings_tensor = embeddings[embedding_modules[embeddings_module]].to(
            ...
            if is_lora_a:
                ...
                if (
                    ...
                    loras[module_name].lora_b = torch.nn.functional.pad(lora_b, (0, 0, 0, addition))
                if pin_memory:
    ...
    def from_local_checkpoint(
        ...
        *,
        ...
        def check_unexpected_modules(modules: dict):
            for lora_module in modules.keys():  # noqa
                ...
                if ".experts" in module_name:
                    if not any(module_name.endswith(ele) for ele in expected_lora_modules):
        ...
        if tensorizer_config_dict:
            ...
            if unexpected_modules and not is_regex_target_modules(peft_helper.target_modules, expected_lora_modules):
                raise ValueError(
                    f"While loading {lora_dir}, expected"
                    f" target modules in {expected_lora_modules}"
                    f" but received {unexpected_modules}."
        ...
        if os.path.isfile(new_embeddings_tensor_path):
            ...
            embeddings = torch.load(new_embeddings_bin_file_path, map_location=device, weights_only=True)
...
class LoRAModelManager:
    ...
    def register_module(self, module_name: str, module: "BaseLayerWithLoRA"):
        assert isinstance(module, BaseLayerWithLoRA), f"Module {module_name} must be a BaseLayerWithLoRA instance,"
        f" got {type(module)}"
        ...
        for module_name, module in self.model.named_modules():
            ...
            if module_name not in self.packed_modules:
                ...
                if parts[-1] in embedding_modules:
                    input_dim = (
                        module.base_layer.org_vocab_size + self.lora_config.lora_extra_vocab_size
                    ...
                    lora = LoRALayerWeights.create_dummy_lora_weights(
                        ...
                        output_dim,
                        rank,
                        module.lora_a_stacked[0].dtype if isinstance(module.lora_a_stacked, list) else module.lora_a_stacked.dtype,
                        "cpu",
                    ...
                    lora = LoRALayerWeights.create_dummy_lora_weights(
                        module_name,
                        module.lora_a_stacked[0].shape[-1] if isinstance(module.lora_a_stacked, (list, tuple)) and len(module.lora_a_stacked) > 0 else module.lora_a_stacked.shape[-1] if not isinstance(module.lora_a_stacked, (list, tuple)) else 0,
                        module.lora_b_stacked[0].shape[-2] if isinstance(module.lora_b_stacked, (list, tuple)) and len(module.lora_b_stacked) > 0 else module.lora_b_stacked.shape[-2] if not isinstance(module.lora_b_stacked, (list, tuple)) else 0,
                        rank,
                        module.lora_a_stacked[0].dtype if isinstance(module.lora_a_stacked, (list, tuple)) and len(module.lora_a_stacked) > 0 else module.lora_a_stacked.dtype if not isinstance(module.lora_a_stacked, (list, tuple)) else torch.float16,
                        "cpu",
                    )
            else:
                parts = module_name.split(".")
                ...
                for i, r in enumerate(replacements):
                    lora = LoRALayerWeights.create_dummy_lora_weights(
                        module_name + "." + r,
                        module.lora_a_stacked[i].shape[-1] if isinstance(module.lora_a_stacked, (list, tuple)) and len(module.lora_a_stacked) > i else module.lora_a_stacked[i].shape[-1] if not isinstance(module.lora_a_stacked, (list, tuple)) else 0,
                        module.lora_b_stacked[i].shape[-2] if isinstance(module.lora_b_stacked, (list, tuple)) and len(module.lora_b_stacked) > i else module.lora_b_stacked[i].shape[-2] if not isinstance(module.lora_b_stacked, (list, tuple)) else 0,
                        rank,
                        module.lora_a_stacked[i].dtype if isinstance(module.lora_a_stacked, (list, tuple)) and len(module.lora_a_stacked) > i else module.lora_a_stacked[i].dtype if not isinstance(module.lora_a_stacked, (list, tuple)) else torch.float16,
                        "cpu",
                    )
...
class LRUCacheLoRAModelManager(LoRAModelManager):
    ...
    def __init__(
        ...
        super().__init__(model, max_num_seqs, max_num_batched_tokens, vocab_size, lora_config, device)
        self._registered_adapters: LoRALRUCache = LoRALRUCache(self.capacity, self.deactivate_adapter)
        self._active_adapters: LoRALRUCache = LoRALRUCache(self.lora_slots, self._deactivate_adapter)
