        ...
        for module_name, module in self.model.named_modules():
            ...
            if module_name not in self.packed_modules:
                ...
                if parts[-1] in embedding_modules:
                    ...
                    lora = LoRALayerWeights.create_dummy_lora_weights(
                        module_name,
                        module.lora_a_stacked[0].shape[-1] if isinstance(module.lora_a_stacked, list) and len(module.lora_a_stacked) > 0 else module.lora_a_stacked.shape[-1] if not isinstance(module.lora_a_stacked, list) else 0,
                        module.lora_b_stacked[0].shape[-2] if isinstance(module.lora_b_stacked, list) and len(module.lora_b_stacked) > 0 else module.lora_b_stacked.shape[-2] if not isinstance(module.lora_b_stacked, list) else 0,
                        rank,
                        module.lora_a_stacked[0].dtype if isinstance(module.lora_a_stacked, list) and len(module.lora_a_stacked) > 0 else module.lora_a_stacked.dtype if not isinstance(module.lora_a_stacked, list) else torch.float16,
                        "cpu",
                    )
            else:
                parts = module_name.split(".")
                ...
                for i, r in enumerate(replacements):
                    lora = LoRALayerWeights.create_dummy_lora_weights(
                        module_name + "." + r,
                        module.lora_a_stacked[i].shape[-1] if isinstance(module.lora_a_stacked, list) and len(module.lora_a_stacked) > i else module.lora_a_stacked[i].shape[-1] if not isinstance(module.lora_a_stacked, list) else 0,
                        module.lora_b_stacked[i].shape[-2] if isinstance(module.lora_b_stacked, list) and len(module.lora_b_stacked) > i else module.lora_b_stacked[i].shape[-2] if not isinstance(module.lora_b_stacked, list) else 0,
                        rank,
                        module.lora_a_stacked[i].dtype if isinstance(module.lora_a_stacked, list) and len(module.lora_a_stacked) > i else module.lora_a_stacked[i].dtype if not isinstance(module.lora_a_stacked, list) else torch.float16,
                        "cpu",
                    )
